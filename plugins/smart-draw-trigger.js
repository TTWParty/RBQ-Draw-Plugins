...
══ 输出 ══
{
  ...
  "segments": [{
    ...
    async function callOpenAiCompatible(messageId, trigger, { signal } = {}) {
        ...
        if (['v5','v6'].includes(store.enhancedContext)) {
            const ecPrompts = {
    ...
    function renderSettings(panel) {
        ...
        container.innerHTML = `
            ...
            <div class="st-scene-trigger-modal-grid">
                ...
                <label class="st-scene-trigger-field" title="选择前情增强分析版本。V2: payload 注入时间线定位。V5/V6: 额外 system prompt 注入中文详细分析。"><span>前情增强分析</span><select id="rbq-sdt-enhanced-context"><option value="off">关闭</option><option value="v2">V2 · 时间线定位</option><option value="v5">V5 · 状态快照</option><option value="v6">V6 · 帧同步 (推荐)</option></select></label>
        ...
        const ecVal = store.enhancedContext === true ? 'v2' : (['v1','v3','v4'].includes(store.enhancedContext) ? 'v2' : (['v7','v8'].includes(store.enhancedContext) ? 'v6' : (store.enhancedContext || 'off')));
