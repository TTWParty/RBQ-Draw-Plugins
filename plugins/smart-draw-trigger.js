        ...
        if (cache) {
            ...
            if (image) {
                ...
                cache.segmentStates[segmentKey].imageResult = {
                    url: image.url && !image.url.startsWith('blob:') ? image.url : (image.displayUrl && !image.displayUrl.startsWith('blob:') ? image.displayUrl : (image.serverOriginalUrl || image.serverUrl || image.serverPreviewUrl || '')),
        ...
        try {
            ...
            for (const item of rendered) {
                ...
                if (restoredResult2 && (restoredResult2.cacheId || restoredResult2.url || restoredResult2.serverUrl || restoredResult2.serverPreviewUrl || restoredResult2.serverOriginalUrl)) {
                    ...
                    const finalUrl2 = restoredResult2.url || restoredResult2.serverOriginalUrl || restoredResult2.serverUrl || restoredResult2.serverPreviewUrl;
                    if (finalUrl2) {
        ...
        if (cached?.shouldDraw && (cached.prompt || cached.segments?.length)) {
            ...
            for (const item of rendered) {
                ...
                if (restoredResult && (restoredResult.cacheId || restoredResult.url || restoredResult.serverUrl || restoredResult.serverPreviewUrl || restoredResult.serverOriginalUrl)) {
                    ...
                    const finalUrl = restoredResult.url || restoredResult.serverOriginalUrl || restoredResult.serverUrl || restoredResult.serverPreviewUrl;
                    if (finalUrl) {
