...
任务：通读小说/对话剧情，地毯式定位每一个该生图的视觉节点，提取独立分镜填入 segments 数组（只要该生图的地方就必须立项，生图数量由视觉节点自然决定，不设固定指标；若包含显式图组标记则 1:1 完整输出全部图组，严禁截断或合并），输出严谨、高审美、解剖自洽的合法 JSON 对象。纯日常闲聊/无视觉画面变化输出 {"shouldDraw": false}。

══ 总则与铁律 ══
- 优先级：先画对（该有的都有）→ 再画稳（锚定复用，跨图连续）→ 后画美（剧情未写处，补充适合氛围的上镜细节，让画面更好看）。
- 视觉节点全覆盖与精准布点准则（核心：分析哪里生图、需要生几张）：
  · 哪里需要生图（视觉全流程节点覆盖法则 · 绝不遗漏）：小说/RP是由连续动态画面组成的，绝不仅有最后的大高潮才算画面！顺着正文时间线自上而下地毯式扫描，凡是出现以下任何一个具备独立画面表现力与叙事价值的节点，每一个节点都必须作为一个独立分镜全部填入 segments 数组，绝严禁偷懒只挑最后一段大高潮而掠过前文所有精彩画面：
    ① 角色造型与服装高光：登场外貌展现、换装、解衣、脱衣暴露、湿身透视、发型散乱等造型亮点；
    ② 动作演进与互动转变：肢体触碰、牵手拥抱、推倒抚摸、动作升级、体位转变、攻守互换；
    ③ 情绪张力与神态特写：动情红晕、咬唇隐忍、落泪、四目相对、眼神拉丝等特写表情；
    ④ 空间机位与氛围转换：场景地点转移、景别与俯仰机位切换（特写/中景/大俯视/大仰视等）；
    ⑤ 显式媒介内容（图组/插画/照片）：正文明确提到的 [图组XX]、[插画]、照片、自拍、手机屏幕等，必须严格 1:1 提取对应数量的分镜，有几个出几个，绝对严禁漏提、截断或合并！
  · 需要生几张（数量自然衍生准则）：生图数量完全由正文包含的独立视觉时刻自然决定，只要正文该生图的地方就必须有图，几张不设死板指标；单一瞬间动作提取 1 个分镜；长文多阶段推进自然拆分为对应数量的独立分镜填入 segments 数组；纯日常闲聊/毫无画面的纯抽象理论探讨才输出 {"shouldDraw": false}。
  · 独立构图与逐字锚定：每个 segment 必须有独立的 label、从当前正文对应段落一字不差截取 10~40 字逐字原文作为 anchor.text、独立的 scene 构图与 characters！
- 跨楼层绝对隔离与防串话铁律（最高红线）：
...
2. 视觉节点全覆盖与精准布点准则（核心：分析哪里生图、需要生几张）：
   - 哪里需要生图（视觉全流程节点覆盖法则 · 绝不遗漏）：小说/RP是由连续动态画面组成的，绝不仅有最后的大高潮才算画面！顺着正文时间线自上而下地毯式扫描，凡是出现以下任何一个具备独立画面表现力与叙事价值的节点，每一个节点都必须作为一个独立分镜全部填入 segments 数组，绝严禁偷懒只挑最后一段大高潮而掠过前文所有精彩画面：
     ① 角色造型与服装高光：登场外貌展现、换装、解衣、脱衣暴露、湿身透视、发型散乱等造型亮点；
     ② 动作演进与互动转变：肢体触碰、牵手拥抱、推倒抚摸、动作升级、体位转变、攻守互换；
     ③ 情绪张力与神态特写：动情红晕、咬唇隐忍、落泪、四目相对、眼神拉丝等特写表情；
...
现在开始处理输入剧情，严格输出合法 JSON 对象！`;
    ...
    const DEFAULTS = {
        ...
        enhancedContext: 'v13', // off | v13 | v14 | v11
...
SCHEMA:
{
  ...
  "characters": [
    ...
    function materializeResultCards(messageId, trigger, result, key) {
        ...
        // 🛡️ 跨楼层幻觉拦截器：严格仅过滤整段（≥14字）完整来自历史楼层且当前楼层完全无关的纯串话分镜
        ...
        const segments = rawSegments.filter((seg) => {
            ...
            // 2. 当前楼层未精确命中时，若当前楼层包含锚点前段或后段（微调/标点差异），绝不拦截
            const cleanA = cleanTextForMatch(anchorText);
            const cleanCurr = cleanTextForMatch(currentMesText);
            if (cleanA.length >= 4 && cleanCurr.length >= 4) {
                const sub4 = cleanA.slice(0, 4);
                const subEnd4 = cleanA.slice(-4);
                if (cleanCurr.includes(sub4) || cleanCurr.includes(subEnd4)) return true;
            }

            // 3. 仅当长文本（≥14字）100% 完整匹配历史楼层，且当前楼层毫无任何重合时，才视为历史串话
            if (cleanA.length >= 14) {
                ...
                const matchesRecentStrict = recentHistoricalTexts.some(histText => {
                    const cleanB = cleanTextForMatch(histText);
                    return cleanB.includes(cleanA);
                });
                if (matchesRecentStrict) {
                    console.warn(`[${PLUGIN_NAME}] 🛡️ 拦截跨楼层串话分镜: 「${seg.label || ''}」整段锚点完全来自上一轮历史对话且当前正文无对应内容，已自动剔除！`, { anchorText });
                    return false;
                }
            }
            return true;
        ...
        const payload = {
            ...
            outputSchema: {
                ...
                segments: ((store.enhancedContext && store.enhancedContext !== 'off') || effectiveMinSeg > 1) ? [
                    {
                        label: 'string (5~15字中文分镜名，如 分镜01·登场/前奏/特定视觉节点)',
                    ...
                    {
                        label: 'string (如 分镜02·后续动作演进/高潮互动，所有该生图的节点均各建独立分镜)',
    ...
    async function maybeAutoGenerate(wrapper, result, messageId, baseKey, segmentKey) {
        const store = getStore();
        const effectiveSegKey = segmentKey || wrapper?.dataset?.rbqSdtSegmentKey || baseKey;
        const state = getSegmentState(store, baseKey, effectiveSegKey, messageId);

        // 🛡️ 核心防御：若未开启自动生图、或已经自动生成过、或已有有效图片结果，绝对不再自动生图
        if (!store.autoRunGenerate) return;
        if (state?.autoGenerated || (state?.imageResult && (state.imageResult.url || state.imageResult.cacheId || state.imageResult.serverUrl))) {
            return;
        }
        ...
        try {
            ...
            RBQ.api.renderInlineGeneratedImage(wrapper, image);
            markSegmentAutoGenerated(baseKey, effectiveSegKey, image, messageId);
            setGenerateButtonState(wrapper, true, getRegenLabel(wrapper), false);
            setWrapperStage(wrapper, 'generated');
        } catch (error) {
            toastr.error(error.message || String(error), PLUGIN_NAME);
    ...
    function renderSettings(panel) {
            ...
            <div id="rbq-sdt-tab-staging" class="rbq-sdt-tab-content">
                ...
                <div class="rbq-sdt-card-group">
                    ...
                    <div class="st-scene-trigger-modal-grid">
                        ...
                        <label class="st-scene-trigger-field" title="前情增强分析：深度推演正文中【哪里需要生图】、【需要生几张】，地毯式识别视觉节点并精准布点。V13: 9.7 全息节拍推演（深度分析生图位置与数量 · 推荐）。V14: 极简四公理自适应推演 (低Token快速推演)。V11: 9.7 全息七步推演 (经典备选)。"><span>前情增强分析</span><select id="rbq-sdt-enhanced-context"><option value="off">关闭 (纯正文直出 · 省Token)</option><option value="v13">V13 · 9.7全息节拍推演 (深度分析生图位置与数量 · 推荐)</option><option value="v14">V14 · 极简四公理自适应推演 (低Token快速推演)</option><option value="v11">V11 · 9.7全息七步推演 (经典备选)</option></select></label>
                    </div>
                </div>
        ...
        document.getElementById('rbq-sdt-enhanced-context').value = ['off', 'v13', 'v14', 'v11'].includes(ecVal) ? ecVal : 'v13';
        ...
        const validPresetKeys = Object.keys(SYSTEM_PROMPT_PRESETS);
        const presetVal = validPresetKeys.includes(store.systemPromptPreset) ? store.systemPromptPreset : DEFAULT_SYSTEM_PROMPT_PRESET;
        document.getElementById('rbq-sdt-system-preset').value = presetVal;
