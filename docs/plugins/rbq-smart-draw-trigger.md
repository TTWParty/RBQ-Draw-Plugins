...
- **🛡️ 大图查看器底部栏触控手势完全隔离**：
  - 滑动底部栏（角色标签、换装按钮、分镜气泡）时彻底阻断 touch 事件向底层查看器冒泡，彻底告别“手指滑动底栏误触发全屏左右切图”的问题。
- **📱 长角色名与多坐标自适应截断保护**：
  - 底部栏角色名称与坐标胶囊增加优雅截断保护（`max-width: min(220px, 52vw)`），并赋予【👗 换装】按钮防挤压特性（`flex-shrink: 0`），长角色名场景下按钮布局依然工整。
...
- **🖼️ 大图查看器底部操作栏弹性自愈兜底**：
  - 彻底修复在大图查看器事件驱动架构下，因宿主底座未显式下发 `bottomBar` 字段导致底部角色信息、立绘动作与分镜气泡完全丢失的 Bug。
  - 引入宿主全局上下文与局部 DOM 容器自动回退机制，确保无论宿主版本新旧，大图查看器底部操作栏均能 100% 稳定高保真渲染。
...
- **🎨 世界书列表 UI 深度修复与排版美化**：
  - **按钮横向紧凑对齐与防折行**：彻底修复已挂载世界书列表中操作按钮（`浏览`、`禁用`、`解绑`）被挤压竖排折行（变成单个汉字竖排）以及卡片被异常拉高的布局缺陷。
  - **紧凑精巧的列表卡片设计**：卡片高度统一优化为紧凑模式（~46px），世界书名称与 entries 数量左侧自适应截断，右侧操作按钮整齐排布，悬浮变色与视觉层级更加精致。
