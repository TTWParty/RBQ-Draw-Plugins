...
══ 输出 ══
{
  ...
  "segments": [{
        ...
        const payload = {
            ...
            ...((ec => {
                const ecPayloads = {
                    ...
                    v7: "SCENE-ENGINE: Before outputting JSON, complete these analysis steps: (1) SCENE SELECTION: identify which moments have the strongest visual impact; if the text references photos/screenshots/video/livestream/media, that content MUST be prioritized as a segment. (2) VIEWPOINT: determine camera type from narrative voice \u2014 first-person narration describing others = pov, external description of multiple characters = third-person, peeping/surveillance/media-within-media = voyeur. This choice dictates how characters array is structured. (3) STATE INHERITANCE: inherit each character's known state from recentMessages, update only what currentMessage explicitly changes. Pose/action from currentMessage only. (4) ACTION ANALYSIS: distinguish momentary vs sustained actions, identify who acts on whom and where results land, assess each character's emotion independently. (5) TEMPORAL: only tag what is currently happening, no completion tags for imminent actions. (6) SEGMENT RELATIONS: when multiple segments exist, determine from text whether they are sequential, parallel, or independent.",
                };
    ...
    async function callOpenAiCompatible(messageId, trigger, { signal } = {}) {
        ...
        if (['v5','v6','v7'].includes(store.enhancedContext)) {
            const ecPrompts = {
                ...
                v7: "【场景引擎】\n在输出 JSON 前，先在脑内完成以下分析：\n\n1. 场景选取：判断当前文本中哪些画面最适合生图。如果文本提到了照片、截图、视频画面、直播等媒介内容，必须优先为该媒介内容生成分镜。纯对话或内心独白无视觉画面时可以不生图。\n\n2. 视角判断：根据叙述视角确定镜头类型。第一人称叙述（"我"/"老子"在看/做）→ pov；外部视角描写多角色互动 → 第三人称；旁观/偷窥/监控/媒介画面 → 旁观视角。视角的选择决定了 characters 的编排方式，务必先确定再生成。\n\n3. 状态继承：从 recentMessages 继承每个角色的已知状态（服装、外貌等），仅当 currentMessage 明确描述变化时才更新。未提及 = 不变。姿势和动作以 currentMessage 为准，不沿用前文。\n\n4. 动作解析：区分瞬间动作和持续动作，选择匹配的 tag。分清谁对谁做了什么，结果发生在谁身上。每个角色的情绪状态单独判断。\n\n5. 时间帧：只 tag 此刻正在发生的事，即将发生的不加完成态 tag。\n\n6. 分镜关系：同一消息多个分镜时，根据正文内容判断它们是时间顺序、并行还是独立的。\n\n核心：每个 tag 都要有文本依据。不超越文本的强度。禁止输出分析文本，只输出 JSON。",
            };
    ...
    function renderSettings(panel) {
        ...
        container.innerHTML = `
            ...
            <div class="st-scene-trigger-modal-grid">
                ...
                <label class="st-scene-trigger-field" title="选择前情增强分析版本。V2: payload 注入时间线定位。V5/V6/V7: 额外 system prompt 注入中文详细分析。"><span>前情增强分析</span><select id="rbq-sdt-enhanced-context"><option value="off">关闭</option><option value="v2">V2 · 时间线定位</option><option value="v5">V5 · 状态快照</option><option value="v6">V6 · 帧同步</option><option value="v7">V7 · 场景引擎 (推荐)</option></select></label>
