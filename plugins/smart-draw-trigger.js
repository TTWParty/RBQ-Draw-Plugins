...
══ 输出 ══
{
  ...
  "segments": [{
        ...
        const payload = {
            ...
            ...((ec => {
                const ecPayloads = {
                    ...
                    v8: "FRAME-SYNC v8: Before outputting JSON, internally reconstruct the visual state of this frame. (1) Inherit unchanged attributes from recentMessages; only update what currentMessage explicitly describes. (2) Determine the precise temporal phase of each action in currentMessage (imminent / ongoing / completed). (3) Identify which passages in currentMessage are visually render-worthy versus pure dialogue/inner monologue. Passages explicitly mentioning images, photos, illustrations, or screen content MUST generate a segment. (4) Determine the camera perspective: whose eyes is this scene viewed through? Apply the matching pov rule from the system prompt. (5) Pose and action come from currentMessage only; distinguish momentary vs sustained actions and mark actor vs recipient. (6) When multiple segments exist, determine their temporal/causal relationship and distribute evenly. Tags must faithfully map the reconstructed state \u2014 never add undescribed changes.",
                };
    ...
    async function callOpenAiCompatible(messageId, trigger, { signal } = {}) {
        ...
        if (['v5','v6','v8'].includes(store.enhancedContext)) {
            const ecPrompts = {
                ...
                v8: "【帧同步分析 V8】\n在输出 JSON 前，先在脑内完成以下分析：\n\n1. 状态继承与更新\n   从 recentMessages 继承每个角色的已知状态（服装、外貌、体型、情绪等）。\n   仅当 currentMessage 明确描述变化时才更新对应属性，未提及 = 不变。\n   每个角色的情绪状态独立判断，不可笼统套用。\n\n2. 时间帧定位\n   判断 currentMessage 中每个动作的精确阶段：即将发生 / 正在进行 / 已经完成。\n   Tag 必须严格匹配该阶段，禁止提前添加结果态标签。\n\n3. 画面筛选与生图必要性\n   扫描 currentMessage 全文，识别哪些段落具有视觉表现力值得生成图片。\n   正文中明确提到「图片」「照片」「配图」「截图」「画面」「视频」「直播」等媒介内容的段落，必须为其生成对应的 segment。\n   纯对话、内心独白、旁白叙述等无视觉变化的段落，不应生成图片。\n   优先级：媒介内容 > 视觉冲击峰值 > 情绪高潮 > 剧情转折。\n\n4. 视角判断\n   根据正文的叙事视角和角色关系，判断当前画面应使用哪种摄像机视角：\n   - 正文以用户(男主/女主)第一人称「我」视角描述 → pov\n   - 正文描述用户观察他人互动 → 旁观/窥视\n   - 正文以客观叙事描述所有角色 → 第三人称\n   视角确定后，按 System Prompt 中对应的视角规则处理角色入场和 Tag 分配。\n\n5. 姿势与动作定位\n   姿势和动作以 currentMessage 为准，不沿用前文的姿势。\n   区分瞬间动作与持续动作，施动者与受动者分别标记。\n\n6. 分镜关系\n   同一消息产生多个 segment 时，根据正文上下文判断它们之间的时序和因果关系。\n   确保各 segment 不重复描述同一画面，且按正文顺序均匀分布。\n\n核心：每个 tag 都要有文本依据。禁止输出分析文本，只输出 JSON。",
            };
    ...
    function renderSettings(panel) {
        ...
        container.innerHTML = `
            ...
            <div class="st-scene-trigger-modal-grid">
                ...
                <label class="st-scene-trigger-field" title="选择前情增强分析版本。V2: payload 注入时间线定位。V5/V6: 额外 system prompt 注入中文详细分析。V8: 新增画面筛选+视角判断。"><span>前情增强分析</span><select id="rbq-sdt-enhanced-context"><option value="off">关闭</option><option value="v2">V2 · 时间线定位</option><option value="v5">V5 · 状态快照</option><option value="v6">V6 · 帧同步</option><option value="v8">V8 · 帧同步增强 (推荐)</option></select></label>
        ...
        const ecVal = store.enhancedContext === true ? 'v2' : (['v1','v3','v4'].includes(store.enhancedContext) ? 'v2' : (store.enhancedContext === 'v7' ? 'v6' : (store.enhancedContext || 'off')));
