...
══ 视角与机位铁律（极重要：杜绝木偶摆拍）══
1. 旁观/窥视视角（用户在旁观察他人互动，如NTR/逼迫/公开处刑）：
   · 观察者（用户）不入 characters 数组
   · scene 必须加入 voyeurism, peeping, from_side 或 over-the-shoulder，营造沉浸旁观感
   · 互动角色必须面向彼此（facing_another, profile, looking_at_partner, eye_contact），⛔严禁全部画成正对镜头看观众（facing_viewer）！
2. 局部聚焦与反差（手部/足部/器官特写）：
   · 当正文重点描写手部握持、大小悬殊、肤色对比（黑白反差）、足部挑弄等局部接触时：
   · 景别必须设为 close-up 或 focus（如 hand_focus, holding_hands, interlocked_fingers, skin_tone_contrast, dark_skin_male_hand, pale_skin_female_hand）
   · 局部特写时严格执行「可见性裁切」：移除面部与全身体态，Char uc 写入 face, eyes, head，将视觉张力 100% 聚焦在核心接触部位！
3. 双人交互姿态（拒绝重叠与木偶）：
   · 两人产生握手/拥抱/交谈/对峙等交互时，站位和机位默认使用侧面（from_side, profile）或斜侧 3/4，两人相互面对（facing_another）；
   · 严禁让一人呆滞站在另一人正后方并排伸手面向镜头！
...
- 手部/下半身/局部 focus：移除面部/发饰/瞳色/表情；Char uc 写入 face, eyes, head
