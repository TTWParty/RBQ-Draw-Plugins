  ...
  "segments": [{
    ...
    function buildRequestPayload(messageId, trigger) {
        ...
        const payload = {
            ...
            ...((ec => {
                const ecPayloads = {
                    v1: "Please implicitly analyze 'recentMessages' to determine the ongoing scene, character relationships, current outfits, and continuous actions, then apply these findings to generate the tags for the 'currentMessage' in the final JSON output. Ensure logical continuity without generating an explicit analysis text.",
                    ...
                    v3: "Implicitly analyze 'recentMessages' for: (1) scene continuity & character states, (2) EXACT temporal moment (imminent/ongoing/completed \u2192 matching tags only), (3) clothing state tracking (only change clothes explicitly described), (4) emotional tone precision (crying\u2260ahegao), (5) body type fidelity. Never fabricate undescribed clothing changes, fluids, or climax states.",
                    v4: "Implicitly analyze 'recentMessages' to extract current state: scene continuity, clothing status per garment, body type, emotional tone, temporal phase (imminent/ongoing/completed), and action progression. Apply these findings to ensure tags accurately reflect the current moment.",
                    ...
                    v6: "FRAME-SYNC PROTOCOL: Before generating JSON, build a per-character state table from recentMessages+currentMessage. For EACH named character: (1) OUTFIT: inherit last known garment state from recentMessages, apply ONLY changes explicitly described in currentMessage (\u2018\u629c\u4e0b\u201d=removed, \u2018\u62c9\u5f00\u2019=open, no mention=unchanged). (2) POSE: use ONLY the pose/position described in currentMessage, never carry over poses from previous messages. (3) EMOTION: map the character\u2019s current emotional state to the correct danbooru tag (\u5c48\u8fb1\u2192humiliation, \u6050\u60e7\u2192fear, \u5feb\u611f\u2192aroused, \u6124\u6012\u2192anger, \u6311\u91c1\u2192smirk\u2014never confuse). (4) SPATIAL: assign center coordinates reflecting the character\u2019s described position relative to the room (\u95e8\u53e3=edge A3/E3, \u4e2d\u95f4=C3, \u65c1\u8fb9=B/D columns). (5) TEMPORAL: tag only what is CURRENTLY happening in this frame\u2014imminent actions get no completion tags. Output ONLY valid JSON.",
                };
    ...
    async function callOpenAiCompatible(messageId, trigger, { signal } = {}) {
        ...
        if (store.enhancedContext === 'v5' || store.enhancedContext === 'v6') {
            const v5Prompt = "【前情增强分析指令】\n在处理 user 传入的 payload 时，你必须首先在脑内对 `recentMessages` 进行隐式分析，建立当前帧的完整状态快照：\n1. 场景连续性：当前空间环境、时间段、氛围基调\n2. 衣态追踪：逐件追踪每个角色的衣物状态（穿着/半脱/脱落/损坏），仅文本明确描述的变化才可更新\n3. 体态与位置：角色的体型特征、当前姿势、空间相对位置\n4. 情绪基调：每个角色此刻的核心情绪（严格区分屈辱/恐惧/快感/愤怒/哀求等，不可混淆）\n5. 时间线定位：当前文本的动作处于哪个阶段——即将发生/正在进行/已经完成，Tag须精确匹配该阶段\n6. 动作承接：上一帧→当前帧之间，什么发生了变化，什么保持不变\n核心原则：生成的Tag必须是当前帧状态快照的忠实映射。文本未描述的变化（衣物/体液/动作/情绪升级）一律不添加。\n最终输出只能是符合 outputSchema 的 JSON，禁止输出任何分析文本。";
            const v6Prompt = "【帧同步协议 · FRAME-SYNC】\n在输出 JSON 前，你必须在脑内对每个具名角色建立状态表：\n\n① 服装状态继承\n  • 从 recentMessages 继承每件衣物的最后已知状态\n  • 仅当 currentMessage 明确描述变化时更新（“解开”=open, “脱下”=removed, “堆到腰间”=lifted）\n  • 未提及的衣物 = 保持上一次状态，禁止自行添加或移除\n  • → 映射到 outfit 字段\n\n② 姿势/动作定位\n  • 仅使用 currentMessage 中描述的姿势，不继承前文姿势\n  • “跪在地毯上”=kneeling, “跨坐在椅背上”=reverse_upright_straddle, “推着小车”=pushing_cart\n  • 复合动作拆解为多个 danbooru tag\n  • → 映射到 action 字段\n\n③ 情绪精确映射\n  • 每个角色独立判断，禁止套用同一情绪\n  • 屈辱→humiliation | 恐惧→fear | 快感→aroused | 愤怒→anger | 挑釁→smirk | 从容→calm | 懵懂→confused\n  • crying≠ahegao, blush≠aroused, calm≠emotionless\n  • → 映射到 action 字段的表情部分\n\n④ 空间布局解析\n  • 根据文本描述的位置关系分配 center 坐标\n  • 门口/边缘 = A列或E列 | 房间中心 = C列 | 旁边 = B/D列\n  • 站立 = 2~3行 | 跪/坐 = 3~4行 | 躺 = 4~5行\n  • 角色间距离要有区分，禁止所有人挤在 C3\n  • → 映射到 center 字段\n\n⑤ 时间帧锁定\n  • 只 tag 此刻正在发生的事，即将发生的动作不加完成态 tag\n  • “正在拉扣子” → unbuttoning，不加 open_clothes\n  • “刚射完” → cum, after_ejaculation，不加 ejaculation\n\n核心约束：每个 tag 都必须能在 currentMessage 或 recentMessages 中找到文本依据。找不到依据的 tag 禁止输出。";
            messages.push({
                role: 'system',
                content: store.enhancedContext === 'v6' ? v6Prompt : v5Prompt
            });
        }
    ...
    function renderSettings(panel) {
        ...
        container.innerHTML = `
            ...
            <div class="st-scene-trigger-modal-grid">
                ...
                <label class="st-scene-trigger-field" title="选择前情增强分析版本。V1~V4: payload 字段注入英文指令。V5/V6: 额外 system prompt 注入中文详细分析。"><span>前情增强分析</span><select id="rbq-sdt-enhanced-context"><option value="off">关闭</option><option value="v1">V1 · 隐式分析 (v4.1.10)</option><option value="v2">V2 · 时间线定位 (v4.2.6)</option><option value="v3">V3 · 5维分析 (v4.2.7a)</option><option value="v4">V4 · 分析导向 (v4.2.7b)</option><option value="v5">V5 · 状态快照 (v4.2.7c)</option><option value="v6">V6 · 帧同步 (推荐)</option></select></label>
                ...
                <label class="st-scene-trigger-field"><span>内置 Prompt 档位</span><select id="rbq-sdt-system-preset"><option value="consistent">V22-完整版</option><option value="storyboarder">V21-POV增强版</option><option value="classic">V20-经典版</option></select></label>
