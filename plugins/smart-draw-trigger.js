        ...
        const payload = {
            ...
            ...((ec => {
                const ecPayloads = {
                    ...
                    v6: "FRAME-SYNC: Before outputting JSON, internally reconstruct each character's current state by diffing recentMessages against currentMessage. Inherit unchanged attributes (outfit, appearance) from context; update only what the current text explicitly changes. Pose and action come from currentMessage only. Emotions must be individually assessed per character. Spatial positions should reflect the narrative layout. Only tag what is happening NOW in this frame. When generating multiple segments within one message, treat them as sequential frames — each segment inherits and builds upon state changes from previous segments in the same message.",
                };
