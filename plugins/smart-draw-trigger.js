...
- 不要把无关系统说明、分析过程、审查声明写进 prompt。`;
    ...
    function bindWrapperManualRun(wrapper, trigger, messageId, key) {
        ...
        button.addEventListener('click', async (event) => {
            ...
            const stage = wrapper.dataset.rbqSdtStage || 'idle';
            if (stage === 'ready-generate') {
    ...
    async function runTaggerForWrapper(wrapper, trigger, messageId, key) {
        ...
        try {
            ...
            ensureTaggerButtonState(wrapper, store.autoRunTagger && RBQ.api.shouldAutoGenerate() ? '等待自动生图...' : '生成图片');
            setWrapperStage(wrapper, 'ready-generate');
